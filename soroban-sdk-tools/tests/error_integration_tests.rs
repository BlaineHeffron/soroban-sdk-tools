...
use soroban_sdk_tools::error::{INNER_BITS, NAMESPACE_MAX};
...
fn from_code_doesnt_match_wrong_namespace() {
    ...
    let namespace = inner_code >> INNER_BITS; // 10/22 bit split
    assert!(
        namespace > 0 && namespace <= NAMESPACE_MAX,
        "Inner variant should have valid namespace bits (1-{}), got {}",
        NAMESPACE_MAX,
        namespace
    );
...
fn test_spec_companion_namespace_codes() {
    ...
    // namespace codes are computed from hash(type_name) % NAMESPACE_MAX + 1.
    // The namespace code is namespace << INNER_BITS (start of range).

    // All namespaces should be valid (1-NAMESPACE_MAX shifted left by INNER_BITS)
    let math_ns = OuterErrorSpec::MathNamespace as u32;
    let namespace = math_ns >> INNER_BITS;
    assert!(
        namespace > 0 && namespace <= NAMESPACE_MAX,
        "Math namespace should be valid (1-{}), got {}",
        NAMESPACE_MAX,
        namespace
    );

    let math_client_ns = OuterErrorSpec::MathClientNamespace as u32;
    let namespace = math_client_ns >> INNER_BITS;
    assert!(
        namespace > 0 && namespace <= NAMESPACE_MAX,
        "MathClient namespace should be valid (1-{}), got {}",
        NAMESPACE_MAX,
        namespace
    );

    let calc_ns = OuterErrorSpec::CalcNamespace as u32;
    let namespace = calc_ns >> INNER_BITS;
    assert!(
        namespace > 0 && namespace <= NAMESPACE_MAX,
        "Calc namespace should be valid (1-{}), got {}",
        NAMESPACE_MAX,
        namespace
    );

    let logic_client_ns = OuterErrorSpec::LogicClientNamespace as u32;
    let namespace = logic_client_ns >> INNER_BITS;
    assert!(
        namespace > 0 && namespace <= NAMESPACE_MAX,
        "LogicClient namespace should be valid (1-{}), got {}",
        NAMESPACE_MAX,
        namespace
    );
    ...
    let namespaces = [
        math_ns >> INNER_BITS,
        math_client_ns >> INNER_BITS,
        calc_ns >> INNER_BITS,
        logic_client_ns >> INNER_BITS,
    ];
...
fn test_build_unified_spec_codes_are_correct_combined_values() {
    ...
    // combined_code = (namespace << INNER_BITS) | inner_code
    //
    // The input codes in the test above are arbitrary u32 values that represent
    // what would be generated by the hash function. The macro simply preserves
    // these values, so we verify that the output matches the input.

    // Test with known input codes that follow the namespace pattern
    let namespace_a: u32 = 100;
    let namespace_b: u32 = 200;
    let inner_a: u32 = 1;
    let inner_b: u32 = 2;

    let code_a1 = (namespace_a << INNER_BITS) | inner_a;
    let code_a2 = (namespace_a << INNER_BITS) | inner_b;
    let code_b1 = (namespace_b << INNER_BITS) | inner_a;

    // Verify extraction works correctly
    assert_eq!(code_a1 >> INNER_BITS, namespace_a);
    assert_eq!(code_a2 >> INNER_BITS, namespace_a);
    assert_eq!(code_b1 >> INNER_BITS, namespace_b);

    // Verify codes from the same namespace share high bits
    assert_eq!(code_a1 >> INNER_BITS, code_a2 >> INNER_BITS);

    // Verify different namespaces have different high bits
    assert_ne!(code_a1 >> INNER_BITS, code_b1 >> INNER_BITS);
}
