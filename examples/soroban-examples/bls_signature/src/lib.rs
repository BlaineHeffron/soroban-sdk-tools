...
use soroban_sdk_tools::{contractstorage, scerr, InstanceItem, PersistentItem};
...
#[contractstorage]
struct Storage {
    owners: PersistentItem<BytesN<96>>,
    counter: InstanceItem<u32>,
    dst: InstanceItem<Bytes>,
}
...
impl IncrementContract {
    pub fn __constructor(env: Env, agg_pk: BytesN<96>) {
        ...
        storage.owners.set(&agg_pk);
        storage.dst.set(&Bytes::from_slice(&env, DST.as_bytes()));
        // initialize the counter, i.e. the business logic this signer contract
        // guards
        storage.counter.set(&0_u32);
    }
...
impl CustomAccountInterface for IncrementContract {
    ...
    fn __check_auth(
        ...
        let storage = Storage::new(&env);
        let agg_pk = storage.owners.get().unwrap();
        let dst = storage.dst.get().unwrap();
