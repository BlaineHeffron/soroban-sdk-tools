...
struct Storage {
    #[short_key = "token_a"]
    token_a: InstanceItem<Address>,
    #[short_key = "token_b"]
    token_b: InstanceItem<Address>,
    #[short_key = "total_shares"]
    total_shares: InstanceItem<i128>,
    #[short_key = "reserve_a"]
    reserve_a: InstanceItem<i128>,
    #[short_key = "reserve_b"]
    reserve_b: InstanceItem<i128>,
    #[short_key = "shares"]
    shares: PersistentMap<Address, i128>,
}

fn get_token_a(e: &Env) -> Address {
    Storage::new(e).token_a.get().unwrap()
}

fn get_token_b(e: &Env) -> Address {
    Storage::new(e).token_b.get().unwrap()
}

fn get_total_shares(e: &Env) -> i128 {
    Storage::new(e).total_shares.get().unwrap()
}

fn get_reserve_a(e: &Env) -> i128 {
    Storage::new(e).reserve_a.get().unwrap()
}

fn get_reserve_b(e: &Env) -> i128 {
    Storage::new(e).reserve_b.get().unwrap()
}
...
fn get_shares(e: &Env, user: &Address) -> i128 {
    Storage::new(e).shares.get(user).unwrap_or(0)
}

fn put_shares(e: &Env, user: &Address, amount: i128) {
    Storage::new(e).shares.set(user, &amount);
}

fn put_token_a(e: &Env, contract: Address) {
    Storage::new(e).token_a.set(&contract);
}

fn put_token_b(e: &Env, contract: Address) {
    Storage::new(e).token_b.set(&contract);
}

fn put_total_shares(e: &Env, amount: i128) {
    Storage::new(e).total_shares.set(&amount)
}

fn put_reserve_a(e: &Env, amount: i128) {
    Storage::new(e).reserve_a.set(&amount)
}

fn put_reserve_b(e: &Env, amount: i128) {
    Storage::new(e).reserve_b.set(&amount)
}
