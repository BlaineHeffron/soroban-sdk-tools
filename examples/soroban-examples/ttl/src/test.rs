...
use super::{Storage, TtlContract, TtlContractClient};
...
fn test_extend_ttl_behavior() {
    ...
    env.as_contract(&contract_id, || {
        ...
        let persistent_key = storage.persistent.get_storage_key();
        let temporary_key = storage.temporary.get_storage_key();
        ...
        assert_eq!(env.storage().persistent().get_ttl(&persistent_key), 499);
        assert_eq!(env.storage().instance().get_ttl(), 499);
        assert_eq!(env.storage().temporary().get_ttl(&temporary_key), 99);
    });
    ...
    env.as_contract(&contract_id, || {
        let storage = Storage::new(&env);
        let persistent_key = storage.persistent.get_storage_key();
        assert_eq!(env.storage().persistent().get_ttl(&persistent_key), 5000);
    });
    ...
    env.as_contract(&contract_id, || {
        let storage = Storage::new(&env);
        let temporary_key = storage.temporary.get_storage_key();
        assert_eq!(env.storage().temporary().get_ttl(&temporary_key), 7000);
    });
    ...
    env.as_contract(&contract_id, || {
        ...
        let persistent_key = storage.persistent.get_storage_key();
        let temporary_key = storage.temporary.get_storage_key();
        assert_eq!(env.storage().persistent().get_ttl(&persistent_key), 0);
        assert_eq!(env.storage().instance().get_ttl(), 5000);
        assert_eq!(env.storage().temporary().get_ttl(&temporary_key), 2000);
    });
...
fn test_temp_entry_removal() {
    ...
    env.as_contract(&contract_id, || {
        let storage = Storage::new(&env);
        let temporary_key = storage.temporary.get_storage_key();
        assert_eq!(env.storage().temporary().has(&temporary_key), false);
    });
}
