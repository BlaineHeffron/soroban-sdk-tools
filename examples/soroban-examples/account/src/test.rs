...
use crate::AccError;
use crate::{AccSignature, AccountContract, AccountContractClient, Storage};
...
fn test_token_auth() {
    ...
    env.as_contract(&account_contract.address, || {
        Storage::new(&env).spend_limits.set(&token, &1000);
    });
