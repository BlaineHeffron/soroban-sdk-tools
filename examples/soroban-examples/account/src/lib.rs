...
use soroban_sdk::{
    ...
    contract, contractimpl, contracttype, crypto::Hash, symbol_short, Address, BytesN, Env, Map,
...
impl AccountContract {
    ...
    pub fn __constructor(env: Env, signers: Vec<BytesN<32>>) {
        ...
        for signer in signers.iter() {
            storage.signers.set(&signer, &());
        }
        storage.signer_count.set(&signers.len());
    }
    ...
    pub fn add_limit(env: Env, token: Address, limit: i128) {
        ...
        Storage::new(&env).spend_limits.set(&token, &limit);
    }
}
...
impl CustomAccountInterface for AccountContract {
    ...
    fn __check_auth(
        ...
        authenticate(&storage, &env, &signature_payload, &signatures)?;

        let tot_signers: u32 = storage.signer_count.get().unwrap();
...
fn authenticate(
    ...
    for i in 0..signatures.len() {
        ...
        if !storage.signers.has(&signature.public_key) {
...
fn verify_authorization_policy(
    ...
    let spend_left: Option<i128> =
        if let Some(spend_left) = spend_left_per_token.get(contract_context.contract.clone()) {
            Some(spend_left)
        } else if let Some(limit_left) = storage.spend_limits.get(&contract_context.contract) {
            Some(limit_left)
        } else {
            None
        };
