...
impl MockToken {
    pub fn initialize(env: &Env, admin: Address, decimal: u32, name: String, symbol: String) {
        let storage = MockTokenStorage::new(env);
        storage.admin.set(&admin);
        storage.decimal.set(&decimal);
    ...
    pub fn mint(env: &Env, to: Address, amount: i128) {
        let storage = MockTokenStorage::new(env);
        let admin: Address = storage.admin.get().unwrap();
        admin.require_auth();

        let current_balance = storage.balances.get(&to).unwrap_or(0);
        storage.balances.set(&to, &(current_balance + amount));
    }

    pub fn balance(env: &Env, id: Address) -> i128 {
        MockTokenStorage::new(env).balances.get(&id).unwrap_or(0)
    }
...
fn setup_test_environment(env: &Env) -> (Address, Address, Address, Address) {
    ...
    (token_id, privacy_pools_id, admin, token_admin)
}
...
fn mock_deposit_auth(
    ...
    let transfer_invoke = MockAuthInvoke {
        contract: token,
        fn_name: "transfer",
        args: (from.clone(), contract.clone(), FIXED_AMOUNT).into_val(env),
        sub_invokes: &[],
    };
    let deposit_invoke = MockAuthInvoke {
        contract,
        fn_name: "deposit",
        args: (from.clone(), commitment.clone()).into_val(env),
...
fn test_deposit_and_withdraw_correct_proof() {
    let env = Env::default();
    let (token_id, contract_id, admin, token_admin) = setup_test_environment(&env);
    ...
    mock_mint_auth(&env, &token_id, &token_admin, &alice, 1000000000);
    ...
    mock_deposit_auth(&env, &contract_id, &token_id, &alice, &commitment);
    ...
    mock_set_association_root_auth(&env, &contract_id, &admin, &association_root);
    ...
    mock_withdraw_auth(&env, &contract_id, &bob, &proof, &pub_signals);
...
fn test_withdraw_insufficient_balance() {
    let env = Env::default();
    let (_token_id, contract_id, admin, _token_admin) = setup_test_environment(&env);
    let client = PrivacyPoolsContractClient::new(&env, &contract_id);
...
fn test_reuse_nullifier() {
    ...
    mock_mint_auth(&env, &token_id, &token_admin, &contract_id, 1000000000);
...
fn test_contract_initialization() {
    let env = Env::default();
    let (_token_id, contract_id, _admin, _token_admin) = setup_test_environment(&env);
    let client = PrivacyPoolsContractClient::new(&env, &contract_id);
...
fn test_withdraw_without_association_set() {
    let env = Env::default();
    let (token_id, contract_id, _admin, token_admin) = setup_test_environment(&env);
...
fn test_withdraw_association_root_mismatch() {
    ...
    mock_set_association_root_auth(&env, &contract_id, &admin, &incorrect_association_root);
...
fn test_set_association_root_non_admin() {
    ...
    mock_set_association_root_auth(&env, &contract_id, &non_admin, &association_root);
