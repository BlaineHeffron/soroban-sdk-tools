...
use soroban_sdk::{
    ...
    contract, contractimpl, log, token, vec, Address, Bytes, BytesN, Env, String, Vec,
};
...
mod groth16_verifier_wasm {
    soroban_sdk::contractimport!(file = "../../fixtures/soroban_groth16_verifier_contract.wasm");
}
...
pub enum Error {
    NullifierUsed,
    InsufficientBalance,
    CoinOwnershipProofFailed,
    OnlyAdmin,
    TreeAtCapacity,
    AssociationRootMismatch,
}
...
impl PrivacyPoolsContract {
    pub fn __constructor(
        ...
        storage.vk.set(&vk_bytes);
        storage.token.set(&token_address);
        storage.groth16_verifier.set(&groth16_verifier);

        // Initialize empty merkle tree with fixed depth
        let tree = LeanIMT::new(env, TREE_DEPTH);
        let (leaves, depth, root) = tree.to_storage();
        storage.tree_leaves.set(&leaves);
        storage.tree_depth.set(&depth);
        storage.tree_root.set(&root);
    }
    ...
    fn store_commitment(env: &Env, commitment: BytesN<32>) -> Result<(BytesN<32>, u32), Error> {
        ...
        let leaves: Vec<BytesN<32>> = storage.tree_leaves.get().unwrap_or(vec![env]);
        let depth: u32 = storage.tree_depth.get().unwrap_or(0);
        let root: BytesN<32> = storage
            .tree_root
            .get()
        ...
        storage.tree_leaves.set(&new_leaves);
        storage.tree_depth.set(&new_depth);
        storage.tree_root.set(&new_root);
    ...
    pub fn deposit(env: &Env, from: Address, commitment: BytesN<32>) -> Result<u32, Error> {
        ...
        let token_address: Address = storage.token.get().unwrap();
    ...
    pub fn withdraw(
        ...
        let vk_bytes: Bytes = storage.vk.get().unwrap();
        ...
        let mut nullifiers: Vec<BytesN<32>> = storage.nullifiers.get().unwrap_or(vec![env]);
        ...
        let state_root: BytesN<32> = storage
            .tree_root
            .get()
            .unwrap_or(BytesN::from_array(env, &[0u8; 32]));
        ...
        let groth16_verifier_id: Address = storage.groth16_verifier.get().unwrap();
        ...
        storage.nullifiers.set(&nullifiers);
        ...
        env.authorize_as_current_contract(vec![
            ...
            InvokerContractAuthEntry::Contract(SubContractInvocation {
                context: ContractContext {
                    ...
                    args: (env.current_contract_address(), to.clone(), FIXED_AMOUNT).into_val(env),
                },
    ...
    fn is_admin(env: &Env, caller: &Address) -> bool {
        let admin: Address = Storage::new(env).admin.get().unwrap();
        *caller == admin
    }
    ...
    pub fn set_association_root(
        ...
        Storage::new(env).association_root.set(&association_root);
    ...
    pub fn get_admin(env: &Env) -> Address {
        Storage::new(env).admin.get().unwrap()
    }
}
