...
impl MerkleDistributionContract {
    ...
    pub fn __constructor(
        ...
        storage.root_hash.set(&root_hash);
        storage.token_address.set(&token);
    ...
    pub fn claim(
        ...
        let root_hash = storage.root_hash.get().unwrap();
        if !root_hash.eq(&hash.to_bytes()) {
            return Err(Error::InvalidProof);
        }

        // Transfer the tokens to the receiver.
        let token = storage.token_address.get().unwrap();
        token::TokenClient::new(&env, &token).transfer(
            &env.current_contract_address(),
            &receiver,
            &amount,
